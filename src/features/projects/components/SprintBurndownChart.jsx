...
export const SprintBurndownChart = ({ data }) => {
    ...
    return (
      <div className="flex items-center justify-center h-64 bg-slate-50 dark:bg-slate-900/30 rounded-2xl border border-dashed border-slate-300 dark:border-slate-700">
        <span className="text-slate-500 dark:text-slate-400 font-medium text-sm">
          No hay datos suficientes para calcular el Burndown
        </span>
      </div>
    );
  }
    ...
    return (
      <div className="flex flex-wrap items-center justify-center gap-6 mt-4 pb-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-bold text-slate-700 dark:text-slate-300">Ritmo Ideal (Lo planeado)</span>
        </div>
        <div className="flex items-center gap-2">
          <div className="w-3 h-3 rounded-full bg-indigo-500 shadow-sm" />
          <span className="text-xs font-bold text-slate-700 dark:text-slate-300">Trabajo Real Pendiente</span>
        </div>
        <div className="flex items-center gap-2">
  ...
  const CustomTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
      return (
        <div className="bg-white dark:bg-[#191c3d] border border-slate-200 dark:border-[#33376b] shadow-xl rounded-xl p-4 min-w-[200px]">
          ...
          <div className="space-y-2">
            {payload.map((entry, index) => {
              ...
              if (entry.dataKey === 'esfuerzo_ideal') {
                labelName = 'Ritmo Ideal';
                color = '#10b981';
              } else if (entry.dataKey === 'esfuerzo_restante') {
                labelName = 'Trabajo Pendiente';
                color = '#6366f1';
      ...
      <ResponsiveContainer width="100%" height="100%">
        ...
        >
          ...
          <YAxis 
            ...
            tick={{ fill: '#64748b', fontSize: 11, fontWeight: 600 }}
            dx={-10}
            label={{ value: 'Puntos de Esfuerzo Restante', angle: -90, position: 'insideLeft', offset: -5, fill: '#64748b', fontSize: 11, fontWeight: 'bold' }}
          />
          ...
          <Line 
            yAxisId="left"
            type="monotone" 
            dataKey="esfuerzo_ideal" 
            stroke="#10b981" 
            strokeWidth={3}
            dot={false}
            name="Ritmo Ideal (Lo planeado)"
          />

          <Line 
            yAxisId="left"
            type="monotone" 
            dataKey="esfuerzo_restante" 
            stroke="#6366f1" 
            strokeWidth={3.5}
            dot={{ r: 4, strokeWidth: 2, fill: '#fff', stroke: '#6366f1' }}
            activeDot={{ r: 6, strokeWidth: 2, fill: '#fff', stroke: '#6366f1' }}
            name="Trabajo Real Pendiente"
          />
        </ComposedChart>
      </ResponsiveContainer>
    </div>
