...
function TeamMatrixView({ selectedProjectId = 'PROJ-01', onSelectDevForScorecard, onNavigateToHealth }) {
            ...
            <table className="w-full text-left text-xs text-slate-300">
              ...
              <tbody className="divide-y divide-slate-800/60">
                {developers.map((dev) => {
                  ...
                  const isTop3 = dev.rank_posicion <= 3;
                    ...
                    >
                      ...
                      <td className="px-4 py-4 text-center">
                        <span className={`inline-flex items-center justify-center w-7 h-7 rounded-full font-bold text-xs ${
                          dev.rank_posicion === 1 ? 'bg-amber-500/20 text-amber-400 border border-amber-500/40' :
                          dev.rank_posicion === 2 ? 'bg-slate-400/20 text-slate-300 border border-slate-400/40' :
                          dev.rank_posicion === 3 ? 'bg-amber-700/20 text-amber-500 border border-amber-700/40' :
                      ...
                      <td className="px-4 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex flex-col">
                            <span className="font-bold text-white text-sm">{dev.nombre}</span>
                      ...
                      <td className="px-4 py-4">
                        <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-semibold ${
                          q.codigo === 'ESTRELLA' ? 'bg-emerald-500/20 text-emerald-300 border border-emerald-500/30' :
                          q.codigo === 'METODICO' ? 'bg-indigo-500/20 text-indigo-300 border border-indigo-500/30' :
                          q.codigo === 'ALTO_VOLUMEN' ? 'bg-amber-500/20 text-amber-300 border border-amber-500/30' :
                          'bg-rose-500/20 text-rose-300 border border-rose-500/30'
                        }`}>
                          {q.codigo === 'ESTRELLA' && '⭐'}
                          {q.codigo === 'METODICO' && '🎯'}
                          {q.codigo === 'ALTO_VOLUMEN' && '⚡'}
                          {q.codigo === 'ATASCADO' && '🐢'}
                      ...
                      <td className="px-4 py-4 text-center">
                        <div className="flex flex-col items-center gap-1">
                          <span className="font-extrabold text-sm text-emerald-400">{dev.performance_score} pts</span>
                      ...
                      {/* THROUGHPUT */}
                      <td className="px-4 py-4 text-center">
                        <span className="font-bold text-white">{dev.throughput_issues}</span>
                        <span className="text-[10px] text-slate-400 block">({dev.velocity_sp} SP)</span>
                      </td>

                      {/* CYCLE TIME */}
                      <td className="px-4 py-4 text-center font-medium">
                        <span className={dev.cycle_time_dias <= teamSummary.team_avg_cycle_time ? "text-emerald-400 font-bold" : "text-amber-400 font-bold"}>
                      ...
                      {/* EXPLICACIÓN DE RESULTADOS ("EL PORQUÉ DE LAS COSAS") */}
                      ...
                      <td className="px-4 py-4 text-center">
                        <button
                          ...
                          className="px-3 py-1.5 text-xs font-semibold rounded-xl bg-indigo-600/20 text-indigo-300 hover:bg-indigo-600 hover:text-white border border-indigo-500/30 transition-all cursor-pointer inline-flex items-center gap-1"
                        >
                          Ver Scorecard
