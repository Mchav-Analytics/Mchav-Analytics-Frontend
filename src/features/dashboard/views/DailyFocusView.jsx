...
export default function DailyFocusView({ selectedProjectId = 'PROJ-01' }) {
  ...
  return (
    <div className="w-full space-y-10 py-4 px-1 text-left font-sans min-h-[85vh] flex flex-col justify-between">
          ...
          <div className="flex items-center gap-5">
            ...
            <div className="space-y-1">
              <h1 className="text-2xl font-bold text-white flex items-center gap-3">
                ...
                <span className="flex items-center gap-2 rounded-full bg-amber-500/10 px-3.5 py-1 text-xs font-semibold text-amber-400 border border-amber-500/20">
      ...
      <div className="group relative flex flex-col rounded-2xl bg-slate-950 p-8 shadow-2xl transition-all duration-300 hover:scale-[1.01] hover:shadow-indigo-500/20 border border-slate-800/80">
        ...
        <div className="relative z-10 space-y-4">
          <div className="flex items-center gap-3 text-indigo-400 font-extrabold text-xs uppercase tracking-wider">
          ...
          <div className="flex flex-wrap gap-6 text-xs font-semibold text-slate-400 pt-4 border-t border-slate-800/80">
            <span className="flex items-center gap-2 text-emerald-400"><TrendingUp size={16} /> Ritmo: +{data?.efficiency_gain_pct || 14}% Eficiencia</span>
            <span className="flex items-center gap-2 text-amber-400"><ShieldCheck size={16} /> Calidad: {data?.clean_deliveries_pct || 100}% Entregas Limpias</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 flex-1">
        ...
        {/* COLUMNA 1: URGENTE */}
        <div className="group relative flex flex-col rounded-2xl bg-slate-950 p-7 shadow-2xl transition-all duration-300 hover:scale-[1.02] hover:shadow-rose-500/20 border border-slate-800/80 justify-between min-h-[260px]">
          ...
          <div className="relative z-10 space-y-6 flex flex-col justify-between h-full">
            <div className="flex items-center justify-between pb-4 border-b border-slate-800">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-xs font-extrabold text-white uppercase tracking-wider">1. Atención Inmediata</h2>
              </div>
              <span className="flex items-center gap-1 rounded-full bg-rose-500/10 px-2.5 py-1 text-[11px] font-bold text-rose-400 border border-rose-500/20">
                {urgentBugs.length} URGENTE
              </span>
            </div>

            {urgentBugs.map((bug, idx) => (
              <div key={idx} className="rounded-xl bg-slate-900/60 p-5 border border-slate-800 space-y-3">
                <div className="flex justify-between items-start">
                  <span className="font-mono text-xs font-bold text-rose-400">{bug.key_issue}</span>
                  <span className="text-[11px] font-bold bg-rose-500/15 text-rose-300 px-2.5 py-1 border border-rose-500/30 rounded-lg">
                    {bug.issue_type || "Bug"}
                  </span>
                </div>
                <h3 className="text-sm font-bold text-slate-100">{bug.summary}</h3>
                <p className="text-xs text-slate-400 leading-relaxed">Devuelto por QA tras encontrar falla en pruebas.</p>
                <div className="pt-3 flex justify-between items-center text-xs text-slate-500 border-t border-slate-800">
        ...
        <div className="group relative flex flex-col rounded-2xl bg-slate-950 p-7 shadow-2xl transition-all duration-300 hover:scale-[1.02] hover:shadow-amber-500/20 border border-slate-800/80 justify-between min-h-[260px]">
          ...
          <div className="relative z-10 space-y-6 flex flex-col justify-between h-full">
            <div className="flex items-center justify-between pb-4 border-b border-slate-800">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-xs font-extrabold text-white uppercase tracking-wider">2. Tarea Activa del Día</h2>
              </div>
              <span className="flex items-center gap-1 rounded-full bg-amber-500/10 px-2.5 py-1 text-[11px] font-bold text-amber-400 border border-amber-500/20">
                EN CURSO
              </span>
            </div>
            ...
            {activeTasks.map((task, idx) => (
              <div key={idx} className="rounded-xl bg-slate-900/60 p-5 border border-slate-800 space-y-3">
                <div className="flex justify-between items-start">
                  <span className="font-mono text-xs font-bold text-indigo-400">{task.key_issue}</span>
                  <span className="text-[11px] font-bold bg-amber-500/15 text-amber-300 px-2.5 py-1 border border-amber-500/30 rounded-lg">
                    {task.story_points || 8} SP
                  </span>
                </div>
                <h3 className="text-sm font-bold text-slate-100">{task.summary}</h3>
                <p className="text-xs text-slate-400 leading-relaxed">Desarrollo principal. Llevas {task.time_spent || "1.8d"} de trabajo activo.</p>
                <div className="pt-3 flex justify-between items-center text-xs text-slate-500 border-t border-slate-800">
                  <span className="flex items-center gap-1.5 text-amber-400"><Clock size={12} /> {task.time_spent || "1.8d / 3.0d"}</span>
                  <button 
        ...
        {/* COLUMNA 3: EN REVIEW */}
        <div className="group relative flex flex-col rounded-2xl bg-slate-950 p-7 shadow-2xl transition-all duration-300 hover:scale-[1.02] hover:shadow-purple-500/20 border border-slate-800/80 justify-between min-h-[260px]">
          ...
          <div className="relative z-10 space-y-6 flex flex-col justify-between h-full">
            <div className="flex items-center justify-between pb-4 border-b border-slate-800">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-xs font-extrabold text-white uppercase tracking-wider">3. En Review</h2>
              </div>
              <span className="flex items-center gap-1 rounded-full bg-purple-500/10 px-2.5 py-1 text-[11px] font-bold text-purple-400 border border-purple-500/20">
            ...
            {reviewTasks.map((rev, idx) => (
              <div key={idx} className="rounded-xl bg-slate-900/60 p-5 border border-slate-800 space-y-3">
                <div className="flex justify-between items-start">
                  <span className="font-mono text-xs font-bold text-purple-400">{rev.key_issue}</span>
                  <span className="text-[11px] font-bold bg-purple-500/15 text-purple-300 px-2.5 py-1 border border-purple-500/30 rounded-lg">
                    {rev.story_points || 13} SP
                  </span>
                </div>
                <h3 className="text-sm font-bold text-slate-100">{rev.summary}</h3>
                <p className="text-xs text-slate-400 leading-relaxed">Esperando aprobación del par técnico en GitHub.</p>
                <div className="pt-3 flex justify-between items-center text-xs text-slate-500 border-t border-slate-800">
