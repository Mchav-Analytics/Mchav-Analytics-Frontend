    ...
    <div className="space-y-6 pb-12 font-sans text-left">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-gradient-to-r from-slate-900 via-indigo-950/40 to-slate-900 p-6 rounded-2xl border border-slate-800 shadow-xl">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <span className="px-3 py-1 text-xs font-bold bg-rose-600/30 text-rose-300 border border-rose-500/40 rounded-full flex items-center gap-1.5">
          ...
          {activeTab === 'system_alerts' ? (
            <div className="flex items-center gap-1 bg-slate-900 p-1 rounded-lg border border-slate-800">
              <button
                ...
                className={`px-2.5 py-1 text-[11px] font-bold rounded-md cursor-pointer ${
                  alertFilter === 'ALL' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'
                }`}
              >
                Todas ({alerts.length})
              </button>
              <button
                ...
                className={`px-2.5 py-1 text-[11px] font-bold rounded-md cursor-pointer ${
                  alertFilter === 'PENDING' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'
                }`}
              >
              ...
              <button
                ...
                className={`px-2.5 py-1 text-[11px] font-bold rounded-md cursor-pointer ${
                  alertFilter === 'DONE' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'
                }`}
              >
            ...
            <div className="flex items-center gap-1 bg-slate-900 p-1 rounded-lg border border-slate-800">
              <button
                ...
                className={`px-2.5 py-1 text-[11px] font-bold rounded-md cursor-pointer ${
                  requestFilter === 'ALL' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'
                }`}
              >
                Todas
              </button>
              <button
                ...
                className={`px-2.5 py-1 text-[11px] font-bold rounded-md cursor-pointer ${
                  requestFilter === 'PENDIENTE' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'
                }`}
              >
                Pendientes
              </button>
              <button
                ...
                className={`px-2.5 py-1 text-[11px] font-bold rounded-md cursor-pointer ${
                  requestFilter === 'EN_ATENCION' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'
                }`}
              >
                En Atención
              </button>
              <button
                ...
                className={`px-2.5 py-1 text-[11px] font-bold rounded-md cursor-pointer ${
                  requestFilter === 'RESUELTA' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'
                }`}
              >
                Resueltas
              </button>
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-800/80 pb-3">
                  <div className="flex items-center gap-2.5">
                    ...
                    {alert.key_issue && (
                      <span className="px-2 py-0.5 text-xs font-mono font-bold bg-slate-800 text-indigo-300 border border-slate-700 rounded">
                        {alert.key_issue}
                      </span>
                    )}
                ...
                <div className="space-y-2">
                  <p className="text-sm font-semibold text-slate-200">{alert.mensaje}</p>
                  {alert.recomendacion && (
                    <div className="bg-indigo-950/40 border border-indigo-500/30 p-3 rounded-xl flex items-start gap-2">
      ...
      {activeTab === 'help_requests' && (
        <div className="space-y-4">
          {filteredRequests.length === 0 ? (
            <div className="bg-slate-900/60 border border-slate-800 p-12 rounded-2xl text-center space-y-3">
              <MessageSquare size={40} className="text-indigo-400 mx-auto" />
              <h3 className="text-base font-bold text-white">No hay solicitudes en esta categoría</h3>
              <p className="text-xs text-slate-400 max-w-md mx-auto">
            ...
            filteredRequests.map((req) => (
              <div
                ...
                className="bg-slate-900/90 border border-slate-800 p-5 rounded-2xl shadow-lg space-y-4 hover:border-slate-700 transition-all text-left"
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-800 pb-3">
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      ...
                      {req.key_issue && (
                        <span className="px-2 py-0.5 text-xs font-mono font-bold bg-slate-800 text-cyan-300 border border-slate-700 rounded">
                          {req.key_issue}
                        </span>
                      )}
                    </div>
                    <h3 className="text-base font-bold text-white pt-1">{req.titulo}</h3>
                  </div>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    {req.estado === 'PENDIENTE' && (
                      <button
                        ...
                        className="px-3 py-1.5 text-xs font-bold bg-amber-600/30 hover:bg-amber-600 text-amber-200 hover:text-white border border-amber-500/40 rounded-lg transition-all flex items-center gap-1.5 cursor-pointer"
                      >
                        <UserCheck size={14} />
                    ...
                    {req.estado !== 'RESUELTA' && (
                      <button
                        ...
                        className="px-3 py-1.5 text-xs font-bold bg-emerald-600/30 hover:bg-emerald-600 text-emerald-200 hover:text-white border border-emerald-500/40 rounded-lg transition-all flex items-center gap-1.5 cursor-pointer"
                      >
                        <CheckCircle2 size={14} />
                ...
                <div className="flex flex-wrap items-center justify-between text-[11px] text-slate-400 gap-2 pt-1 border-t border-slate-800/60">
                  <div className="flex items-center gap-4">
                    <span>
                      Solicitado por: <strong className="text-slate-200">{req.solicitado_por_name}</strong> ({req.rol_usuario})
                    </span>
                    <span>Email: <span className="text-indigo-300">{req.solicitado_por_email}</span></span>
                  </div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
          <div className="bg-slate-900 border border-slate-700 w-full max-w-lg rounded-2xl shadow-2xl p-6 space-y-4 text-left">
            ...
            <form onSubmit={handleCreateRequestSubmit} className="space-y-4">
              <div className="space-y-1">
                <label className="text-xs font-semibold text-slate-300">Título de la Solicitud *</label>
                <input
                  type="text"
                  required
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="text-xs font-semibold text-slate-300">Ticket Requerido (Opcional)</label>
                  <input
                    type="text"
              ...
              <div className="space-y-1">
                ...
                <textarea
                  ...
                  className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-xs text-white outline-none focus:border-indigo-500 leading-relaxed"
                />
              </div>
