...
interface AdminUserTableProps {
  ...
  totalPages: number;
  itemsPerPage: number;
  handleRoleChange: (userId: string, role: 'ADMIN' | 'MANAGER' | 'DEVELOPER' | 'DESACTIVADO') => void;
  toggleUserStatus: (userId: string) => void;
  approveUser?: (userId: string, role?: 'ADMIN' | 'MANAGER' | 'DEVELOPER' | 'DESACTIVADO') => void;
}
          ...
          >
            <div className="grid grid-cols-1 xl:grid-cols-[minmax(220px,1.4fr)_minmax(160px,1fr)_minmax(180px,1.1fr)_minmax(140px,0.9fr)_110px] gap-4 xl:gap-5 items-center px-5 py-5">
              <div className="flex items-center gap-4 min-w-0">
                <div className={`w-11 h-11 rounded-full flex items-center justify-center font-black text-xs text-white shrink-0 ring-2 shadow-sm ${u.role === 'ADMIN' ? 'bg-gradient-to-br from-purple-500 to-purple-700 ring-purple-300 dark:ring-purple-500/40' :
                  u.role === 'MANAGER' ? 'bg-gradient-to-br from-blue-500 to-blue-700 ring-blue-300 dark:ring-blue-500/40' :
                    u.role === 'DESACTIVADO' ? 'bg-gradient-to-br from-slate-500 to-slate-700 ring-slate-300 dark:ring-slate-500/40' :
                ...
                <div className="min-w-0 space-y-1">
                  <h4 className="text-sm font-extrabold text-slate-900 dark:text-slate-100 truncate flex items-center gap-2 flex-wrap">
                    ...
                    {u.role === 'DESACTIVADO' && (
                      <span className="px-2 py-0.5 rounded-md bg-rose-100 dark:bg-rose-500/20 text-rose-700 dark:text-rose-300 text-[10px] font-mono font-bold border border-rose-200 dark:border-rose-500/30">
                        DESACTIVADO
                      </span>
                    )}
              ...
              <div className="flex flex-col gap-1.5 xl:items-center">
                ...
                <select
                  value={u.role}
                  onChange={e => handleRoleChange(u.id, e.target.value as 'ADMIN' | 'MANAGER' | 'DEVELOPER' | 'DESACTIVADO')}
              ...
              <div className="flex flex-col gap-2 xl:items-center">
                ...
                <div className="flex flex-wrap items-center gap-2 xl:justify-center">
                  <span className={`px-3 py-1.5 rounded-full text-[11px] font-extrabold inline-flex items-center gap-2 ${u.status === 'ACTIVE' && u.role !== 'DESACTIVADO'
                  ...
                  {u.status === 'INACTIVE' || u.role === 'DESACTIVADO' ? (
                    <button
                      onClick={() => approveUser ? approveUser(u.id, u.role === 'DESACTIVADO' ? 'DEVELOPER' : u.role) : toggleUserStatus(u.id)}
